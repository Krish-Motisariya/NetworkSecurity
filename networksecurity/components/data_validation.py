...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Required nuber of columns : {number_of_columns}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"trained datframe does not contain all columns"
            status = self.validate_number_of_columns(test_dataframe)
            if not status:
                error_message = f"test datframe does not contain all columns"
